...
def main():
    """Performs least-squares bending-wave analysis for waves 1, 2, and 5.

    Loads the precomputed CWT/ridge file produced by ``make_cwt_and_ridge``,
    extracts each wave's radial segment, estimates local vertical amplitude
    via :func:`reconstruction.theta_max_from_IF`, reconstructs the full
    bending-wave geometry, and compares the photometric model to the data.
    ...
    3. Mask out wave 1, 2, and 5 radial segments; locally re-normalize
       wave 5 brightness to its mean.
    4. Compute per-segment sliding-window amplitude ``A_V`` for each wave
       using :func:`reconstruction.theta_max_from_IF`.
    5. Plot and save ``A_V`` vs. radius for all three waves.
    6. Reconstruct the full bending-wave displacement, tilt, and I/F model
       via :func:`reconstruction.reconstruct_bending_wave` and plot the
       results.
    7. Fit the photometric model to observed I/F via
    ...
    w5_mask = (r > config.WAVE5_RADIUS_MIN) & (r < config.WAVE5_RADIUS_MAX)
    ...
    wave_5_rs = r[w5_mask]
    wave_5_ks = k_est_smooth[w5_mask]
    wave_5_raw = b_norm[w5_mask]

    # wave 5: detrend locally
    """
    poly_deg = 2
    coef = np.polyfit(wave_5_rs, wave_5_IF, poly_deg)
    wave_5_base = np.polyval(coef, wave_5_rs)
    wave_5_base = np.where(np.abs(wave_5_base) < 1e-6, np.nan, wave_5_base)
    """
    wave_5_IF = wave_5_raw / np.average(wave_5_raw)


    # matching geometry
    cotB_eff_1 = cotB_eff_profile[w1_mask]
    cotB_eff_2 = cotB_eff_profile[w2_mask]
    cotB_eff_5 = cotB_eff_profile[w5_mask]

    # amplitude windows
    L1 = round((2 * np.pi) / np.average(wave_1_ks))
    L2 = round((2 * np.pi) / np.average(wave_2_ks))
    L5 = round((2 * np.pi) / np.average(wave_5_ks))
    ...
    _, _, wave_5_Amp = theta_max_from_IF(
        wave_5_rs,
        wave_5_ks,
        wave_5_IF,
        L=L5,
    ...
    ax5.plot(wave_5_rs, np.abs(wave_5_Amp * 1000))
    ax5.set_title("Wave 5 LS Amplitude (A_V)")
    ax5.set_xlabel("Radius (km)")
    ax5.set_ylabel("Amplitude (m)")
    plt.tight_layout()
    amp5_path = os.path.join(config.PLOTS_DIR  + "/LS", config.IMG_PRFX + "Wave_5_LS_Amplitude.png")
    fig5.savefig(amp5_path, dpi=200)
    plt.show()
    ...
    wave_5_rec = reconstruct_bending_wave(
        wave_5_rs,
        wave_5_ks,
        wave_5_Amp,
        m=1,
    ...
    plot_bending_wave(wave_5_rec, which=("z", "tan_theta", "IF_model"))
    ...
    res5 = reconstruct_and_compare_IF(
        wave_5_rs, wave_5_ks, wave_5_Amp, wave_5_IF,
        cotB_eff=cotB_eff_5,
        IF0=1.0,
    )
    ...
    B_eff_fit_5_deg = (np.arctan(1.0 / res5["cotB_eff_fit_mean"]))
    ...
    B_eff_true_5 = float(np.nanmean(B_eff_profile[w5_mask]))
    ...
    print(f"Wave 5: RMSE = {res5['rmse']:.4g}  R² = {res5['r2']:.3f}  "
          f"corr = {res5['corr']:.3f}  B_eff_fit ≈ {B_eff_fit_5_deg:.3f}°  "
          f"(true ≈ {B_eff_true_5:.3f}°)")
    plot_IF_fit(res5, title="Wave 5 LS Reconstruction")
