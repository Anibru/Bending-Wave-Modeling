...
import os
import config
from utils import smooth_moving_average
...
def plot_bending_wave(out, which=("z", "tan_theta", "IF_model"), titles=None, save_dir=config.PLOTS_DIR, prefix="bending_wave"):
    ...
    Args:
        out (dict): Output dictionary from :func:`reconstruct_bending_wave`.
        ...
        titles (dict[str, str] | None): Optional mapping from key name to
            ...
            titles are used.
        save_dir (str): Directory in which to save the figure.  Defaults to
            ``config.PLOTS_DIR``.
...
def reconstruct_and_compare_IF(r, k_cycles, A_V, IF,
    ...
    Args:
        ...
        cotB_eff (float | array-like): Cotangent of the effective elevation
            angle; can be a scalar or a 1-D array of length ``N``.
            Defaults to ``1.0``.
        IF0 (float | None): Baseline I/F.  If ``None``, the mean of ``IF``
    ...
    Returns:
        dict: A dictionary containing:
            ...
            - ``"r"`` (numpy.ndarray): Radial positions.
            - ``"y"`` (numpy.ndarray): Observed residual ``IF - IF0``.
            - ``"y_hat"`` (numpy.ndarray): Model fit ``a*Xc + b*Xs``.
            - ``"IF0"`` (float): Baseline level used.
    ...
    # effective photometric scale actually used
    geom_used = cotB_eff * np.hypot(a, b)
    # make a scalar summary so printing doesn't choke
...
def plot_IF_fit(result, title="", save_dir=config.PLOTS_DIR, prefix=""):
    """Plots observed vs modeled I/F residuals and saves the figure.

    Produces a two-panel figure:

    - **Upper panel**: observed residual ``y = IF - IF0`` versus the model
    ...
    Args:
        result (dict): Output dictionary from
            :func:`reconstruct_and_compare_IF`, expected to contain keys
            ``"r"``, ``"y"``, ``"y_hat"``, ``"IF0"``,
            ``"cotB_eff_input"`` (optional), and
            ``"cotB_eff_fit_mean"`` (optional).
        title (str): Figure title and component of the output filename.
        ...
        prefix (str): Optional filename prefix prepended before ``title``.
            Defaults to ``""``.
    """
    os.makedirs(save_dir, exist_ok=True)
    ...
    ax[0].plot(r, y, label=f"Measured residual = IF - {IF0:.3f}", alpha=0.8)
    ...
    # save automatically before showing
    save_path = os.path.join(save_dir, config.IMG_PRFX + f"{prefix}_{title.replace(' ', '_')}.png")
    fig.savefig(save_path, dpi=200)
