# 01_make_cwt_and_ridge_NORM.py
# This file is the same as make_cwt_and_ridge.py,
# but it accepts-pre-normalized data
import os
import numpy as np
import matplotlib
...
def main():
    ...
    Identical pipeline to ``make_cwt_and_ridge.main()`` except that the
    global normalization step is skipped: the ``"norm_val"`` field is read
    directly from the ``.sav`` file and used as ``b_norm``.

    Execution steps:

    1. Load the IDL ``.sav`` profile from ``config.DATA_PATH`` and read
       ``"norm_val"`` as the pre-normalized brightness array.
    2. Run :func:`wavelet_cwt.run_cwt_and_ridge` over the configured
