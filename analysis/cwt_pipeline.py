# analysis/cwt_pipeline.py
"""Loads a pre-normalized brightness profile, runs CWT ridge detection, and saves results."""
...
import config
from helpers.utils import edges_from_centers
from helpers.cwt import prepare_power, run_cwt_and_ridge


def main():
    """Loads a pre-normalized brightness profile, runs CWT ridge detection, and saves results.

    Reads the ``"norm_val"`` field directly from the ``.sav`` file as the
    pre-normalized brightness array (no additional normalization step is
    applied).

    Execution steps:

    1. Load the IDL ``.sav`` profile from ``config.DATA_PATH`` and read
       ``"norm_val"`` as the pre-normalized brightness array.
    2. Run :func:`helpers.cwt.run_cwt_and_ridge` over the configured
       bandwidth grid (no ridge masking applied).
    3. Plot the wavelet power spectrum and the raw/smoothed ridge trace,
       saving both figures to ``config.PLOTS_DIR``.
    4. Save all CWT/ridge arrays to ``config.OUT_DIR/cwt_ridge_result.npz``
    ...
    # 1) load pre-normalized profile
    data = readsav(config.DATA_PATH)
    r = data["radi"]
    b_norm = data["norm_val"]
    ...
    ax.set_title(f"Rev {config.IMG_REV} · Image {config.IMG_NUM} – Wavelet Power ({wavelet_best})")
    ...
    ax2.set_title(f"Rev {config.IMG_REV} · Image {config.IMG_NUM} – Dominant Wavenumber")
    ax2.legend()
    plt.tight_layout()
