# Train NN to predict complex amplitude components (A_c, A_s) on wave 2 segment
...
def main():
    """Trains a neural-network bending-wave model on the wave 2 radial segment.

    Identical workflow to ``NN_wave_1_analysis.main()`` targeting the wave 2
    segment (``config.WAVE2_RADIUS_MIN`` to ``config.WAVE2_RADIUS_MAX``).
    Smoothing loss coefficients are larger (1st-derivative: 5e3,
    2nd-derivative: 5e2) to enforce smoother amplitude estimates over wave 2.

    Execution steps:

    1. Load ``r``, ``b_norm``, and ``k_est_smooth_best`` from
       ``config.CWT_FILE``.
    2. Mask the wave 2 radial segment and fix byte order for PyTorch.
    ...
    # 2) wave-2 mask FIRST
    # ------------------------------------------------
    mask = (r > config.WAVE2_RADIUS_MIN) & (r < config.WAVE2_RADIUS_MAX)
    ...
    plot_IF_fit(result, title="Wave 2 NN Reconstruction", save_dir=config.PLOTS_DIR + "/NN")
    ...
    out_path = os.path.join(config.OUT_DIR, config.IMG_PRFX + "nn_wave2_amp.npz")
    np.savez(
        out_path,
        r=r_w,
        A_V=A_v_final,
    ...
    ax1.set_title("Wave 2 NN Amplitude (A_V)")
    ax1.set_xlabel("Radius (km)")
    ax1.set_ylabel("Amplitude (m)")
    plt.tight_layout()
    amp1_path = os.path.join(config.PLOTS_DIR + "/NN", config.IMG_PRFX + "Wave_2_NN_Amplitude.png")
    ...
    ax1c.set_title("Wave 2 NN Cosine Amplitude (A_c)")
    ax1c.set_xlabel("Radius (km)")
    ax1c.set_ylabel("Amplitude (m)")
    plt.tight_layout()
    amp1c_path = os.path.join(config.PLOTS_DIR + "/NN", config.IMG_PRFX + "Wave_2_NN_Cosine_Amplitude.png")
    ...
    ax1s.set_title("Wave 2 NN Sine Amplitude (A_s)")
    ax1s.set_xlabel("Radius (km)")
    ax1s.set_ylabel("Amplitude (m)")
    plt.tight_layout()
    amp1s_path = os.path.join(config.PLOTS_DIR + "/NN", config.IMG_PRFX + "Wave_2_NN_Sine_Amplitude.png")
    ...
    ax2.plot(r_w, psi_np)
    ax2.set_title("Wave 2 NN psi")
    ax2.set_xlabel("Radius (km)")
    ax2.set_ylabel("Phase (rad)")
    plt.tight_layout()
    psi_path = os.path.join(config.PLOTS_DIR + "/NN", config.IMG_PRFX + "Wave_2_NN_psi.png")
    ...
    ax3.plot(r_w, k_w)
    ax3.set_title("Wave 2 NN k")
    ax3.set_xlabel("Radius (km)")
    ax3.set_ylabel("Wave Number (rad/km)")
    plt.tight_layout()
    k_path = os.path.join(config.PLOTS_DIR + "/NN", config.IMG_PRFX + "Wave_2_NN_k.png")
